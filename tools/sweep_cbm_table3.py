...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run CBM sweeps on paper3737 and aggregate paper-style metrics.")
    ...
    p.add_argument("--min-matches", type=str, default="20", help="Comma-separated min-matches values.")
    p.add_argument(
        "--match-distance-thr",
        type=str,
        default="4.0",
    ...
    p.add_argument("--tag-prefix", type=str, default=None, help="Optional prefix under outputs/cbm/tmp/ for shards.")
...
def run_many(cmds: List[List[str]], *, env: dict, workers: int, dry_run: bool) -> None:
    ...
    if failures:
        ...
        raise RuntimeError(
            f"{len(failures)}/{len(cmds)} CBM shards failed; "
...
def main() -> None:
    ...
    min_matches_list = _parse_int_list(args.min_matches)
    ...
    base_env.update({
        ...
        # CBM uses torch; forcing CPU avoids GPU OOM when running many shards in parallel.
    ...
    for noise in noises:
        for min_matches in min_matches_list:
            for thr in thr_list:
                ...
                out_base = f"paper3737_noise{noise}_min{min_matches}_{thr_tag}_{init_tag}_{icp_tag}"
                for shard_idx, (start, end) in enumerate(ranges):
                    out_tag = f"tmp/{prefix}/{out_base}_chunk{shard_idx:03d}"
                    out_metrics = REPO_ROOT / "outputs" / "cbm" / out_tag / "metrics.json"
    ...
    total_jobs = len(noises) * len(min_matches_list) * len(thr_list)
    ...
    for noise in noises:
        for min_matches in min_matches_list:
            for thr in thr_list:
                ...
                out_base = f"paper3737_noise{noise}_min{min_matches}_{thr_tag}_{init_tag}_{icp_tag}"
                aggregate(
                    f"outputs/cbm/tmp/{prefix}/{out_base}_chunk*/matches.jsonl",
                    REPO_ROOT / "outputs" / "cbm" / out_base,
                    thresholds="1,2,3",
                )
    print("Done.")
