...
def build_common_args(
    ...
    *,
    ...
    if force_pose_confidence is not None:
        args.append(f"--force-pose-confidence {float(force_pose_confidence)}")
    return args
...
def build_tasks(
    *,
    ...
    def _best_selection_args() -> List[str]:
        policy = str(pose_selection_policy or "compare_current_proxy").strip().lower()
        args = [f"--pose-selection-policy {policy}"]
        if policy == "compare_current_proxy":
            args.extend(
                [
        ...
        for sweep in sweeps:
            ...
            for noise, rot in zip(noise_list, rot_list):
                common = build_common_args(
                    ...
                    force_pose_confidence=force_pose_confidence,
                )
                if include_baseline:
                ...
                for method_name in methods:
                    ...
                    args = [f"--pose-correction {meta[initfree]}", f"--note {note}"]
                    args.extend(_best_selection_args())
            ...
            if force_pose_confidence is not None:
                common.append(f"--force-pose-confidence {float(force_pose_confidence)}")
            if include_single:
...
def write_config_snapshot(out_dir: Path, args: argparse.Namespace, noise_list: Sequence[str], rot_list: Sequence[str]):
    cfg = {
        ...
        "pose_selection_policy": str(getattr(args, "pose_selection_policy", "compare_current_proxy")),
        ...
        "force_pose_confidence": (
            None if getattr(args, "force_pose_confidence", None) is None else float(args.force_pose_confidence)
        ),
